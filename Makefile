.PHONY: dev lint complex coverage pre-commit yapf format sort deploy destroy deps unit infra-tests integration e2e coverage-tests docs lint-docs build
...
lint: format
	@echo "Running flake8"
	flake8 service/* cdk/* tests/* docs/examples/* --exclude patterns='node_modules,build,cdk.json,cdk.context.json,.yaml'
	@echo "Running mypy"
	$(MAKE) mypy-lint

complex:
	@echo "Running Radon"
	radon cc -e 'tests/*,cdk.out/*,node_modules/*' .
	@echo "Running xenon"
	xenon --max-absolute B --max-modules A --max-average A -e 'tests/*,.venv/*,cdk.out/*,node_modules/*' .
